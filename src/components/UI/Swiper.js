    ...
    if (partOne + partTwo + partThree === 12) {
      return (
        <View style={styles.progressButton}>
          <ProgressButton text="Get My Score" onPress={this.modalHandler} />
        </View>
      );
    }
    return (
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        <SwiperFlatList ref="swiper" showPagination index={0}>
          {content}
        </SwiperFlatList>
        <SubmitTest
...
const mapDispatchToProps = dispatch => {
  return {
    onAddAnswer: (answer, testNumber, index, part) =>
      dispatch(addAnswer(answer, testNumber, index, part)),
